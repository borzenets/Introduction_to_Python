...
def start():
    while True:
        ...
        match menu_choice:
            ...
            case 2:
                search_string = view.get_search_string()
                search_result = model.search(search_string, phone_book)
                view.show_contacts(search_result, 'Нет совпадений')
            case 3:
            ...
            case 4:
                 view.show_contacts(phone_book, 'Телефонная книга пуста')
                 date, index = view.edit_contact()
                 phone_book = model.add_remove_change_contact(phone_book, data=date, index=index)
                 line_p_book = model.pars_dict_in_list_to_lines_csv(phone_book)
                 model.file_db(patch, 'w', line_p_book)
